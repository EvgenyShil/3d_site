        ...
        <div className="space-y-4">
          ...
          {dims && (
            <div className="card p-4 text-sm">
              <div>
                Габариты (единицы модели): {dims.x.toFixed(1)} × {dims.y.toFixed(1)} × {dims.z.toFixed(1)}
              </div>
        ...
        <div className="space-y-4">
          <div className="card p-4 space-y-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => setParams((p) => ({ ...p, material: e.target.value as MaterialCode }))}
              >
                {MATERIALS.map((m) => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  step="0.04"
                  className="input"
                  value={params.layerMm}
                  onChange={(e) => setParams((p) => ({ ...p, layerMm: parseFloat(e.target.value) }))}
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  className="input"
                  value={params.infillPct}
                  onChange={(e) => setParams((p) => ({ ...p, infillPct: parseInt(e.target.value, 10) }))}
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="input"
                  value={params.qty}
                  onChange={(e) => setParams((p) => ({ ...p, qty: parseInt(e.target.value, 10) }))}
                />
              </div>
              <div className="flex items-end gap-2">
                <input
                  id="urgent"
                  type="checkbox"
                  checked={params.urgency}
                  onChange={(e) => setParams((p) => ({ ...p, urgency: e.target.checked }))}
                />
                <label htmlFor="urgent" className="label">
                  Срочно
                </label>
              </div>
            </div>
                  ...
                  <label key={k} className="badge cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        setParams((p) => ({
                          ...p,
                          postprocess: e.target.checked
                            ? [...p.postprocess, k]
                            : p.postprocess.filter((x) => x !== k),
                        }))
                      }
                    />{' '}
                    {k}
            ...
            <div className="flex gap-2">
              <button className="btn-primary" onClick={onQuote} disabled={volumeCm3 == null}>
                Рассчитать
              </button>
            ...
            {quote && (
              <div className="text-sm text-zinc-700">
                Оценка: {quote.unit} ₽/шт · Итого: {quote.total} ₽
              </div>
            )}
          </div>
          ...
          <div className="card p-4">
            ...
            <p className="text-sm text-zinc-600 mt-1">
              Печатаем на принтерах с термокамерой → высокая прочность межслойной адгезии и стабильная геометрия, меньше
              коробления.
            </p>
          </div>
        </div>
